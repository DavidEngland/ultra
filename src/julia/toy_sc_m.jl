...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    # --- Save results for notebook consumption ---
    using DelimitedFiles
    # times is a Vector; ensure column vectors and write header-friendly CSV
    out = hcat(collect(times), Ts_hist, T1_hist, ice_h_hist)
    csv_path = joinpath(@__DIR__, "..", "ML", "toy_sc_m_results.csv")  # ../ML relative to julia/
    ...
    writedlm(csv_path, out, ',')
    println("Saved results -> ", csv_path)
end